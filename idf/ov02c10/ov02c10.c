...
/* Local change: the gain table is generated, not carried over from upstream.
 * Measured on the module in the Guition JC1060P470 with a fixed long exposure:
 *   0x3508:0x3509  ("analogue gain" in the Linux driver) - no effect on the
 *                  picture in any encoding or mode-register setting tried;
 *   0x350a:0x350b  ("digital gain" in the Linux driver, 1/256 x per LSB) -
 *                  brightens as expected.
 * So the table drives 0x350a:0x350b from 1.0x to 15.9x and leaves 0x3508 at
 * 1.0x. Upstream declared 63x through 0x3508 and delivered about 4x. */
